...
COPY_REVIEW
   ...
   ├──→ COPY_CHANGES_REQUESTED ─→ DRAFTING
       ...
       ART_PREPARING
         ...
         QA_REVIEW
          ...
          └──→ FINAL_REVIEW
                    ...
                    ├──→ RENDER_CHANGES_REQUESTED ─→ RENDERING
...
- an explicit human change request;

- prior copy approval, if any, is invalid for the new revision.
...
Use only when the requested change is visual/layout/art and does not alter approved factual copy.
...
If the requested change affects factual or editorial copy, transition to `COPY_CHANGES_REQUESTED` instead.
